############################################################################
############################################################################
##########   A reanalysis of Folt et. al 2022 using 20 ages     ##########
##########               K. Loope, July 2023                      ##########
############################################################################
############################################################################
...
#KJL run options [2 lines]
cluster=T
test=F

#KJL note: this version uses a parallelized version of the original for loop across scenarios.
# Set options
options(scipen=999)
set.seed(999)


#KJL set up versions (only one in this case) [6 lines]
this.version = 1
startwd<-getwd()
version<-c("3percent-unrolled")[this.version] #specify the version of metapopulation growth
dir.create(version,showWarnings=F)#make output folder for this version
versionwd<-paste0(startwd,"/",version)
...
#KJL added tidyr, doParallel, popbio
packages = c("tidyr","installr", "raster", "sp", "elevatr", "lognorm", "plyr", "plotrix",
             "clipr", "ggmap", "ggplot2", "maptools", "dplyr", "ggsn",
             "mapdata", "reshape", "ggrepel", "patchwork", "png","doParallel","popbio") # Specify the packages of interest
...
package.check <- lapply( # Load or install+load all packages
  ...
  FUN = function(x) {
    if (!require(x, character.only = TRUE)) {
      ...
      library(x, character.only = TRUE, warn.conflicts = F, quietly = T)
    }
  }
)
...
#KJL commented out to reduce logfile output
#head(datum)
...
saveRDS(datum, "3percent-unrolled/clean-tortoise-data.rds")
...
for (u in 1:length(intervals)){
  ...
  r = if(test){2}else{50}             # Number of simulation replicates  KJL: set it to 2 if doing a test
  ...
  #KJL set it to first two rows if doing a test [2 lines]
  if(test) {Scenarios=Scenarios[1:2,]}
  if(test) {nscens=2}
  ...
  ## KJL set up base transition matrix and diagonals [8 lines]
  nages=20
  agenames<-paste0("age_",1:20)
  basemat <- matrix(0,nrow=nages,ncol=nages)
  rownames(basemat) = agenames
  colnames(basemat) = agenames

  mat_ndx <- matrix(1:nages^2,ncol=nages)
  ndx_subdiags <- diag(mat_ndx[-1,-ncol(mat_ndx)])


  ### A) Future scenario loop
  #KJL initialize cluster with 32 cores if on cluster, 2 cores if not [5 lines]
  ncores=if(cluster){32} else{2}
  myCluster <- makeCluster(spec=ncores,type="PSOCK")    # PREPARE FOR RUNNING PARALLEL REPLICATES
  registerDoParallel(myCluster)
  par_out<-foreach(g=1:length(Scenarios[,1]),.packages = c('popbio','lognorm',"dplyr","reshape","tidyr"))%dopar% {
    #for (g in 1:length(Scenarios[,1])){ #KJL commented out in favor of foreach above.
    #KJL create a list for each scenario's outfiles (will be 457 long, one for each pop) [1 line]
    outlist<-list()
    ...
    #KJL: this was reading densityDependence as a character, skipping the density dependence if statement later on [1 line]
    densityDependence = as.numeric(Scenarios$DensityDependence[g])
    ...
    for (h in c(1:sites)){    # Subset to research site, h
      ...
      #KJL Nages - abundance of females across age classes [1 line]
      Nages = array(0,c(20,r,t))
      #KJL add a matrix for keeping track of age at maturity [1 line]
      thisMA = matrix(0,r,t)
      #finally, make another array that keeps track of our transition matrix [1 line]
      mat_array = array(0,c(20,20,r,t))
      #also make a matrix to keep track of the metapopulation lambda [1 line]
      LAM = matrix(0,r,t)
      ...
      if (mImm > 0){  # Immigration = 0 can't be modeled with error
        ...
        AImmi = matrix(0,r,1)
        BImmi = matrix(0,r,1)
        Immt = matrix(0,r,t)} else {Immt = matrix(0,r,t)
        }
      ...
      if(test) {r=2} #KJL set r to 2 if testing
      for (i in 1:r){
        ...
        for(j in 1:t){
          ...
          # Maturation rates
          Tjait[i,j] = 1/(rlnorm(1,getParmsLognormForMoments(Tjai[i]+Mslope*warming*j/t,sdTja^2)[1],getParmsLognormForMoments(Tjai[i]+Mslope*warming*j/t,sdTja^2)[2])-1)
          # calculated by 1 / (age of maturity - 1)
          # subtract one from denominator to remove hatchling year from juv stage

          #KJL: this year's age at maturity, capping it at 20 [1 line]
          thisMA[i,j] = min(round(1/Tjait[i,j] + 1),20)
          ...
          #KJL: add Nages to zero [1 line]
          if (elevit[i,j] < 0){density[i,j] = 0; Na[i,j] = 0; eggs[i,j]=0; Nj[i,j] = 0; Nimm[i,j] = 0; Nages[,i,j] = 0} else {
            ...
            #print(sprintf("density: %s",density[i,j]))
            ...
            # KJL make the transition matrix for j = 1[31 lines]
            #if first year, need to make it for j = 1 and estimate stable age structure for juvenile ages
            #then populate age classes based on Na[i,1] and Nj[i,1], with all adults going to age_20, and juvies distributed from
            # 1:(thisMA[i,1]-1) in proportion to the stable age class from the matrix of the first year
            if(j == 1){
              thismat = basemat
              #set reproduction for mature adults (prob repro * clutch size * nest survival * hatch success * prob female * hatchling survival)
              #this is taken from brian's egg calculator, removing the number of adults term, and adding hatchling survival from last year too
              thismat[1,thisMA[i,j]:nages] =  Pfbt[i,j]*Pnst[i,j]*Fit[i,j]*Pht[i,j]*PropFt[i,j]*Sht[i,j]#thisPR*thisCS*thisNS*thisHS*thisPF*thisPHI_H
              #set the survivals for juvies from class 1 thru age at maturity - 1
              thismat[ndx_subdiags[1:(thisMA[i,j]-1)]] = Sjt[i,j]

              #set the survivals for adults
              thismat[ndx_subdiags[thisMA[i,j]:nages]] = Sat[i,j]
              thismat[nages,nages] = Sat[i,j]

              ssd<-popbio::stable.stage(thismat)

              ages.init<-rep(0,length=length(Nages[,i,1]))
              names(ages.init)<-names(ssd)
              #make adjusted probabilities of each juv and adult age class so each sums to one
              ssd.j.adjust<-(ssd[1:(thisMA[i,j]-1)]/sum(ssd[1:(thisMA[i,j]-1)]))
              ssd.a.adjust<-(ssd[thisMA[i,j]:nages]/sum(ssd[thisMA[i,j]:nages]))

              #so, let's randomly assign each individual to an age class, with probabilities relative to the stable age distribution (separate for juv and adult)
              jtemp<-table(names(ssd.j.adjust)[sample(c(1:length(ssd.j.adjust)), size = Nj[i,1], replace = TRUE, prob = ssd.j.adjust)])
              ages.init[names(jtemp)]<-jtemp
              atemp<-table(names(ssd.a.adjust)[sample(c(1:length(ssd.a.adjust)), size = Na[i,1], replace = TRUE, prob = ssd.a.adjust)])
              ages.init[names(atemp)]<-atemp

              #now store them in Nages
              Nages[,i,1]<-ages.init

            }
            # KJL make the transition matrix for j > 1 [22 lines]
            if(j>1){
              #KJL: get our base matrix
              thismat = basemat
              #set reproduction for mature adults (prob repro * clutch size * nest survival * hatch success * prob female * hatchling survival)
              #this is taken from brian's egg calculator, but putting j to the previous year and removing the number of adults term, and adding hatchling survival from last year too
              thismat[1,thisMA[i,j]:nages] =  Pfbt[i,j-1]*Pnst[i,j-1]*Fit[i,j-1]*Pht[i,j-1]*PropFt[i,j-1]*Sht[i,j-1]#thisPR*thisCS*thisNS*thisHS*thisPF*thisPHI_H
              #set the survivals for juvies from class 1 thru age at maturity - 1
              thismat[ndx_subdiags[1:(thisMA[i,j-1]-1)]] = Sjt[i,j-1]

              #set the survivals for adults
              thismat[ndx_subdiags[thisMA[i,j-1]:nages]] = Sat[i,j-1]
              thismat[nages,nages] = Sat[i,j-1]

              #stochastic projection: do survival/reproduction/emigration (using apparent survival)
              Nages[1,i,j] <- rpois(1,sum(thismat[1,]*Nages[,i,j-1])) #do repro first
              for(age in 2:nages){ #do the "promotion" from one stage to the next
                Nages[age,i,j]<-rbinom(1,Nages[age-1,i,j-1],thismat[age,age-1]) #binomial: n = number of individuals of previous stage in prevoius time step; prob = the subdiagonal (survival)
              }
              #store the number promoted to adults
              new_adults<-Nages[thisMA[i,j-1],i,j]
              #now add in the survivors from last year's last stage (stage 20)
              Nages[nages,i,j]<-Nages[nages,i,j]+rbinom(1,Nages[nages,i,j-1],thismat[nages,nages]) #add in binomial with n = number of stage 20 individuals in previous time step, prob = survival
            }
            #KJL save our transition matrix [1 line]
            mat_array[,,i,j]<-thismat

            # KJL add immigrants to oldest age class.  could distribute them randomly across mature age classes, but i don't see a benefit to doing that [3 lines]
            if(j>1){
              Nages[nages,i,j]<-Nages[nages,i,j]+round(Nimm[i,j-1])
            }




            #KJL reset to last year's adult values if density has hit density dependence [5 lines]
            if (j>1){
              if(density[i,j-1] > densityDependence){
              Nages[thisMA[i,j-1]:nages,i,j] <- round(Nages[thisMA[i,j-1]:nages,i,j-1]*Sat[i,j-1])
              }
            }
            #KJL we will continue to use Na and Nj to keep track of adults and juveniles for Brian's other calculations [4 lines]
            if(j>1){
              Na[i,j]<-sum(Nages[thisMA[i,j]:nages,i,j])
              Nj[i,j]<-sum(Nages[1:(thisMA[i,j]-1),i,j])
            }
            #KJL comment out original Na and Nj calculator
            # Adults
            ...
            #KJL make a transition matrix for the original approach (note: only needed for comparing lambdas; code not implemented in this version) [9 lines]
            if(j>1){
              origmat<-matrix(c(Sjt[i,j-1]*(1-Tjait[i,j-1]), #upper left element is prob of juvenile last year staying juvenile: juv survival * 1-prob transitioning
                                Pfbt[i,j-1]*Pnst[i,j-1]*Fit[i,j-1]*Pht[i,j-1]*PropFt[i,j-1]*Sht[i,j-1], #upper right element is recruitment from adult stage into juv stage: product of our 6 fecundity component parameters, including hatchling survival
                                Sjt[i,j-1]*Tjait[i,j-1], #Lower left element: prob of juvenile becoming adult: juv survival * trans prob
                                Sat[i,j-1]),nrow=2,byrow=T) #lower right element: prob of adult surviving: adult survival
            }else{ #gotta use j instead of j-1 for j = 1 year
              origmat<-matrix(c(Sjt[i,j]*(1-Tjait[i,j]),Pfbt[i,j]*Pnst[i,j]*Fit[i,j]*Pht[i,j]*PropFt[i,j]*Sht[i,j],
                                Sjt[i,j]*Tjait[i,j],Sat[i,j]),nrow=2,byrow=T)
            }
            ...
            #KJL Put a 3% cap on metapop lambda [6 lines]
            if(version == "3percent-unrolled"){
              if (j>1){
                if(!is.na(LAM[i,j]) & (LAM[i,j]>1.03)){
                  LAM[i,j]<-1.03
                }
              }
            }
      ...
      ### KJL For each population * scenario, save the new objects so they are saved but not stored in memory [16 lines]
      out<-data.frame(
                      ma=as.vector(thisMA),
                      i=rep(1:r,80),
      ...
      #KJL now do Nages [2 lines]
      temp<-cbind(expand.grid(age=paste0("age_",1:20),rep=1:r,t=1:80),abund=as.vector(Nages))
      temp<-tidyr::pivot_wider(temp,values_from="abund",names_from="age")

      #KJL bring in Nages into out [2 lines]
      out<-cbind(out,temp)
      out<-out[with(out, order(i, j)),]
      #KJL save it to the list [1 lines]
      outlist[[h]]<-out
    ...
    #KJL now save the output (one for each scenario) [2 lines]
      if(!dir.exists(sprintf("%s/output",version))) dir.create(sprintf("%s/output",version),showWarnings = F)
    ...
    #KJL specify reshape, as this has the names flipped from dplyr::rename, messing with my mind!
    popsH = reshape::rename(popsH, c("meanF"="MeanFecundity", "meanTja"="MeanAgeMaturity",
    ...
    #KJL: for dopar, return things we care about in a format we can then do.call rbind together after the loop [4 lines]
    this_scen_out<-c(c(lapply(ScenSummUnits,function(x) x[g,]),lapply(ScenExtantTracker,function(x) x[g,])),
                     list(ScenSumm[g,],ScenarioResults[[g]]))
    names(this_scen_out)<-c("ScenSummUnits1","ScenSummUnits2","ScenSummUnits3","ScenSummUnits4","ScenSummUnits5","ScenExtantTracker1","ScenExtantTracker2","ScenExtantTracker3","ScenSumm","ScenarioResults")
    return(this_scen_out)
} # Close the scenario loop

  #KJL [16 lines]
  #now recreate our output objects after the dopar loop
  ScenSummUnits <-list()
  ScenSummUnits[[1]]<-do.call("rbind",lapply(par_out,function(x) x$ScenSummUnits1))
  ScenSummUnits[[2]]<-do.call("rbind",lapply(par_out,function(x) x$ScenSummUnits2))
  ScenSummUnits[[3]]<-do.call("rbind",lapply(par_out,function(x) x$ScenSummUnits3))
  ScenSummUnits[[4]]<-do.call("rbind",lapply(par_out,function(x) x$ScenSummUnits4))
  ScenSummUnits[[5]]<-do.call("rbind",lapply(par_out,function(x) x$ScenSummUnits5))

  ScenExtantTracker<-list()
  ScenExtantTracker$ExtantMean<-do.call("rbind",lapply(par_out,function(x) x$ScenExtantTracker1))
  ScenExtantTracker$ExtantLCL<-do.call("rbind",lapply(par_out,function(x) x$ScenExtantTracker2))
  ScenExtantTracker$ExtantUCL<-do.call("rbind",lapply(par_out,function(x) x$ScenExtantTracker3))

  ScenSumm <- do.call("rbind",lapply(par_out,function(x) x$ScenSumm))

  ScenarioResults <-lapply(par_out,function(x) x$ScenarioResults)
  ...
  #KJL specify that it's from the 20ages version [1 line]
  saveRDS(RESULTS, paste0(version,"/pva-20ages-output-results-",80,"yr.rds"))
#KJL compress the output files [3 lines]
  setwd(versionwd)
tar("output.tar.gz", "output", compression = 'gzip')
setwd(startwd)

  #### Runtimes:
  #### for t = 80, takes about ~2.1 hours
  ##KJL on cluster with 32 cores, updated code takes ~10 minutes

} # End interval

#KJL stop the cluster [1 line]
stopCluster(myCluster)

# Stop the clock
proc.time() - ptm

#KJL for log file [1 line]
print("done with model")

#KJL #don't make plots and tables if this is just a test
if(!test){
  #######################################################################
  ########## Section 4)                                        ##########
  ...
  # KJL Read in results of the population projections [16 lines]
  #using _orig to signal original results
  #these come from a version of the original code [output by tortoise-pva_response.R].
  RESULTS80_orig = readRDS("original/pva-output-results-80yr.rds")
  names(RESULTS80_orig$ScenarioResults) = RESULTS80_orig$ScenSummTotal[,1]

  #KJL comment these out
  ##head(RESULTS80_orig)
  ##head(RESULTS80_orig$ScenSummTotal) #Scenarios
  ##head(RESULTS80_orig$ScenSummUnits) #Genetic units
  ##head(RESULTS80_orig$ScenPopsExtantTracker) #Extinction tracker by scenario
  ##head(RESULTS80_orig$ScenarioResults) #Results for each population in each scenario

  ScenSummTotal_orig = RESULTS80_orig[[1]]
  ScenSummUnits_orig = RESULTS80_orig[[2]]
  ScenExtantTracker_orig= RESULTS80_orig[[3]]
  ScenarioResults_orig = RESULTS80_orig[[4]]


  # KJL Read in results of the new version.  use _ages to indicate new model [14 lines]
  RESULTS80_ages = readRDS(paste0(version,"/pva-20ages-output-results-80yr.rds"))
  names(RESULTS80_ages$ScenarioResults) = RESULTS80_ages$ScenSummTotal[,1]

  #KJL comment these out
  ##head(RESULTS80_ages)
  ##head(RESULTS80_ages$ScenSummTotal) #Scenarios
  ##head(RESULTS80_ages$ScenSummUnits) #Genetic units
  ##head(RESULTS80_ages$ScenPopsExtantTracker) #Extinction tracker by scenario
  ##head(RESULTS80_ages$ScenarioResults) #Results for each population in each scenario

  ScenSummTotal_ages = RESULTS80_ages[[1]]
  ScenSummUnits_ages = RESULTS80_ages[[2]]
  ScenExtantTracker_ages= RESULTS80_ages[[3]]
  ScenarioResults_ages= RESULTS80_ages[[4]]


# KJL Plot end results of original vs ages model [entire section edited]------------------------
  #get extant pops for original
  ExtantPops_orig = melt(ScenExtantTracker_orig$ExtantMean)
  LCL_orig = melt(ScenExtantTracker_orig$ExtantLCL)
  UCL_orig = melt(ScenExtantTracker_orig$ExtantUCL)
  ExtantPops_orig = cbind(ExtantPops_orig,LCL_orig[,3],UCL_orig[,3])
  colnames(ExtantPops_orig) = c("Scenario","Year","PopsExtant","LCL","UCL")


  #get for ages
  ExtantPops_ages = melt(ScenExtantTracker_ages$ExtantMean)
  LCL_ages = melt(ScenExtantTracker_ages$ExtantLCL)
  UCL_ages = melt(ScenExtantTracker_ages$ExtantUCL)
  ExtantPops_ages = cbind(ExtantPops_ages,LCL_ages[,3],UCL_ages[,3])
  colnames(ExtantPops_ages) = c("Scenario","Year","PopsExtant","LCL","UCL")


  #KJL label them and combine
  ExtantBoth=rbind(ExtantPops_orig,ExtantPops_ages)
  ExtantBoth$model<-c(rep("original",times=nrow(ExtantPops_orig)),rep("ages",times=nrow(ExtantPops_ages)))

  ExtantEnd = subset(ExtantBoth,Year==80)
  ExtantEnd$ScenarioNum<-as.numeric(ExtantEnd$Scenario)
  pdf(paste0(version,"/figure0-ages_vs_orig_final_pops.pdf"),width=6,height=4)
  ggplot(ExtantEnd, aes(x=1, y=PopsExtant,fill=model)) +
    geom_bar(position=position_dodge(), stat="identity", colour='black') +
    geom_errorbar(aes(ymin=LCL, ymax=UCL), width=.2,position=position_dodge(.9))+
    facet_wrap(ScenarioNum~.,nrow=4) +theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())+xlab("")
  dev.off()
  #ggsave("figure0-ages_vs_orig_final_pops.pdf",width=6,height=4)
  ...
  #KJL add "_ages" at end of ResULTS80 [1 line]
  table2 = RESULTS80_ages$ScenSummTotal[,1]

  # Copy and paste table into Excel for more formal formatting
  #write_clip(table2)
  #KJL write as csv instead [1 line]
  write.csv(table2,paste0(version,"/table2.csv"))
  ...
  #KJL add _ages at end of object name [1 line]
  table3 = RESULTS80_ages$ScenSummTotal[,c("Scenario", "NToti","NTott","NTottLCL","NTottUCL","NTottDelta",
                                             "PopsTi","PopsTt","PopsTtLCL","PopsTtUCL","PopsTDelta",
                                             "NMetaI","NMetaT","NMetaTLCL","NMetaTUCL","NMetaTDelta")]
  #write_clip(table3)
  #KJL save as CSV [1 line]
  write.csv(table3,paste0(version,"/table3.csv"))
  ...
  #KJL add _ages at end of object name [1 line]
  table4 = RESULTS80_ages$ScenSummTotal[c(1:length(table2)),][,c("Scenario", "Extant","ExtantPerc","VLikelyExtant","VLikelyExtantPerc",
                                                                   "LikelyExtant","LikelyExtantPerc","UnlikelyExtant","UnlikelyExtantPerc")]
  #write_clip(table4)
  #save as csv [1 line]
  write.csv(table4,paste0(version,"/table4.csv"))
  ...
  #KJL add _ages at end of object name [13 lines]
  app2 = rbind(RESULTS80_ages$ScenSummUnits[[1]][,c("NToti","NTott","NTottLCL","NTottUCL","NTottDelta",
                                                      "PopsTi","PopsTt","PopsTtLCL","PopsTtUCL","PopsTDelta",
                                                      "NMetaI","NMetaT","NMetaTLCL","NMetaTUCL","NMetaTDelta")],
               RESULTS80_ages$ScenSummUnits[[2]][,c("NToti","NTott","NTottLCL","NTottUCL","NTottDelta",
                                                      "PopsTi","PopsTt","PopsTtLCL","PopsTtUCL","PopsTDelta",
                                                      "NMetaI","NMetaT","NMetaTLCL","NMetaTUCL","NMetaTDelta")],
               RESULTS80_ages$ScenSummUnits[[3]][,c("NToti","NTott","NTottLCL","NTottUCL","NTottDelta",
                                                      "PopsTi","PopsTt","PopsTtLCL","PopsTtUCL","PopsTDelta",
                                                      "NMetaI","NMetaT","NMetaTLCL","NMetaTUCL","NMetaTDelta")],
               RESULTS80_ages$ScenSummUnits[[4]][,c("NToti","NTott","NTottLCL","NTottUCL","NTottDelta",
                                                      "PopsTi","PopsTt","PopsTtLCL","PopsTtUCL","PopsTDelta",
                                                      "NMetaI","NMetaT","NMetaTLCL","NMetaTUCL","NMetaTDelta")],
               RESULTS80_ages$ScenSummUnits[[5]][,c("NToti","NTott","NTottLCL","NTottUCL","NTottDelta",
                                                      "PopsTi","PopsTt","PopsTtLCL","PopsTtUCL","PopsTDelta",
                                                      "NMetaI","NMetaT","NMetaTLCL","NMetaTUCL","NMetaTDelta")])
  ...
  #write_clip(app2)
  #KJL save as CSV instead [1 line]
  write.csv(app2,file=paste0(version,"/appendix2.csv"))
  ...
  ScenarioNumber = rep(c(1:nscens), t)
  #KJL change to "_orig" [9 line]
  ExtantPops_orig = cbind(ScenarioNumber,ExtantPops_orig)

  ExtantPops_orig$Label <- NA
  ExtantPops_orig$Label[which(ExtantPops_orig$Year == max(ExtantPops_orig$Year))] <- 1:32
  ExtantPops_orig$Label2 <- NA
  ExtantPops_orig$Label2 <-  rep(1:nscens, 80)
  ExtantPops_orig$Scenario2 <- paste(ExtantPops_orig$Label2," = ", ExtantPops_orig$Scenario)
  ExtantPops_orig$Scenario2 <- factor(ExtantPops_orig$Scenario2,
  ...
  colnames(ExtantPops_orig) = c("ScenarioNumber","Scenario1","Year","PopsExtant","LCL","UCL","Label","Label2","Scenario")

  # Panel A) Sensitivity analysis of single-factor scenarios
  nA = 21 # number of scenarios for panel A
  #KJL add _orig to object name [1 line]
  resultsA = droplevels(subset(ExtantPops_orig, ExtantPops_orig$ScenarioNumber %in% c(1:nA)))
  ...
  #KJL add _orig to object name [1 line]
  resultsB = droplevels(subset(ExtantPops_orig, ExtantPops_orig$ScenarioNumber %in% c((nA+1):(nA+nB))))
  ...
  # plot(figure5ab)
  # ggsave("figure5_orig.png", width=12, height=12)
  # ggsave("figure5_tall_orig.png", width=10, height=16)

  # Open file with Microsoft Paint, and manually adjust panel A legend
  #   to be more flush with the graph, like in Panel B

  ## Plot the two maps side by side
  #KLJ add _mod, save as original file for side by side compariosn [4 lines]
  figure5ab_mod = fig5a + fig5b + plot_layout(ncol=2, nrow=1)
  pdf("3percent-unrolled/figure5_orig.pdf", width=12, height=12)
  plot(figure5ab_mod)
  dev.off()

  #ggsave("figure5_wode_tall_orig.png", width=10, height=16)


#KJL new version of Fig 5 using the output from the age-based unrolled model [120 lines]
  ########## Figure 5 - ages ############
  ...
  ExtantPops_ages #from above
  ScenarioNumber = rep(1:nscens, t)
  ExtantPops_ages = cbind(ScenarioNumber,ExtantPops_ages)

  ExtantPops_ages$Label <- NA
  ExtantPops_ages$Label[which(ExtantPops_ages$Year == max(ExtantPops_ages$Year))] <- 1:32
  ExtantPops_ages$Label2 <- NA
  ExtantPops_ages$Label2 <-  rep(1:nscens, 80)
  ExtantPops_ages$Scenario2 <- paste(ExtantPops_ages$Label2," = ", ExtantPops_ages$Scenario)
  ExtantPops_ages$Scenario2 <- factor(ExtantPops_ages$Scenario2,
                                        levels=c('1  =  Status quo','2  =  Survival (high)','3  =  Survival (low)','4  =  Survival (very low)',
                                                 '5  =  Max density (high)','6  =  Max density (low)','7  =  Immigration (very high)','8  =  Immigration (high)',
                                                 '9  =  Immigration (zero)','10  =  Climate warming (low)','11  =  Climate warming (medium)','12  =  Climate warming (high)',
                                                 '13  =  Sea-level rise (low)','14  =  Sea-level rise (medium)','15  =  Sea-level rise (high)','16  =  Urbanization (low)',
                                                 ...
                                                 '29  =  Survival (low) + medium threats','30  =  Immigration (very high) + medium threats',
                                                 '31  =  Immigration (high) + medium threats','32  =  Immigration (zero) + medium threats'))
  colnames(ExtantPops_ages) = c("ScenarioNumber","Scenario1","Year","PopsExtant","LCL","UCL","Label","Label2","Scenario")
  names(ExtantPops_ages)
  # Panel A) Sensitivity analysis of single-factor scenarios
  nA = 21 # number of scenarios for panel A
  resultsA_ages = droplevels(subset(ExtantPops_ages, ExtantPops_ages$ScenarioNumber %in% c(1:nA)))

  (fig5a_ages = ggplot(data=resultsA_ages, aes(x=Year, y=PopsExtant, group=Scenario)) +
      geom_line(aes(color=Scenario), lwd=1.3) +
      geom_line(aes(y=UCL, color=Scenario), lwd=0.8, linetype="dashed") +
      geom_line(aes(y=LCL, color=Scenario), lwd=0.8, linetype="dashed") +
      theme(legend.position=c(0.15, 0.33),
  ...
  # Panel B) Future prediction analysis for multi-factor scenarios
  nB = 11 # number of scenarios for panel A
  resultsB_ages = droplevels(subset(ExtantPops_ages, ExtantPops_ages$ScenarioNumber %in% c((nA+1):(nA+nB))))
  (fig5b_ages = ggplot(data=resultsB_ages, aes(x=Year, y=PopsExtant, group=Scenario)) +
      geom_line(aes(color=Scenario), lwd=1.3) +
      geom_line(aes(y=UCL, color=Scenario), lwd=0.8, linetype="dashed") +
      geom_line(aes(y=LCL, color=Scenario), lwd=0.8, linetype="dashed") +
      theme(legend.position=c(0.15, 0.33),
  ...
  ## Plot the two maps side by side
  #figure5ab_ages = fig5a_ages + fig5b_ages + plot_layout(ncol=1, nrow=2)
  #plot(figure5ab_ages)
  #ggsave("figure5_ages.png", width=12, height=12)
  #ggsave("figure5_tall_ages.png", width=10, height=16)

  # Open file with Microsoft Paint, and manually adjust panel A legend
  #   to be more flush with the graph, like in Panel B

  ## Plot the two maps side by side
  figure5ab_mod_ages = fig5a_ages + fig5b_ages + plot_layout(ncol=2, nrow=1)

  pdf(paste0(version,"/figure5_wide_ages.pdf"), width=12, height=12)
  plot(figure5ab_mod_ages)
  dev.off()
  #ggsave("figure5_wode_tall_ages.png", width=10, height=16)

  fig5a_nolegend=fig5a+
    theme(legend.position="none")+ggtitle("original")+
    theme(plot.title = element_text(size = 32, face = "bold"))

  fig5b_nolegend=fig5b+
    theme(legend.position="none")+ggtitle("original")+
    theme(plot.title = element_text(size = 32, face = "bold"))

  fig5a_ages=fig5a_ages+ggtitle("ages")+
    theme(plot.title = element_text(size = 32, face = "bold"))

  fig5b_ages=fig5b_ages+ggtitle("ages")+
    theme(plot.title = element_text(size = 32, face = "bold"))

  figure5ab_both = fig5a_nolegend + fig5b_nolegend +fig5a_ages + fig5b_ages + plot_layout(ncol=2, nrow=2)

  pdf(paste0(version,"/figure5_both.pdf"), width=12, height=16)
  plot(figure5ab_both)
  dev.off()
  # Open file with Microsoft Paint, and manually adjust panel A legend
  #   to be more flush with the graph, like in Panel B
  ...
  #head(RESULTS80_ages$ScenarioResults[[26]]) # Results for 'Management (less) + medium threats' scenario
  #KJL add _ages to object name [1 line]
  prediction = RESULTS80_ages$ScenarioResults[[26]] # Results for medium threats + less management
  ...
  # Number of Extremely Likely to Persist or Very Likely to Persist
  colSums(table(prediction$PPCat, prediction$GeneticUnits)[c(1:2),])

  # Total number of populations
  colSums(table(prediction$PPCat, prediction$GeneticUnits))
  ...
  fig6a <- state_map +
    ...
    geom_point(aes(x=i-0.8, y=j+0.5), colour = "black", fill="yellow", shape=24, size=5) +
    geom_point(aes(x=i-0.8, y=j+0), colour = "black", fill="orange", shape=23, size=5) +
  ...
  #KJL save CSV file of units x persistance probability [1 line]
  write.csv(table(prediction$PPCat, prediction$GeneticUnits),paste0(version,"/units_by_extinction.csv"))
...
#KJL save in version folder [3 lines]
  pdf(paste0(version,"/figure6.pdf"), width=14, height=6.5)
  plot(figure6)
  dev.off()
