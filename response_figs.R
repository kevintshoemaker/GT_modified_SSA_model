...
#First, plot some examples demonstrating runaway growth with and without denstiy dependence failure

# Figure S2 - Example plot of DD failure and exponential growth -----------


#setwd("/Users/kevinloope/Dropbox/GopherTortoise/Analysis/PVA/FoltModel2023/Code/")
#look at one example case
this.scen=5  # max density high

temp_orig<-readRDS(sprintf("original/output/scen-%s.rds",this.scen)) #read in the replicate-level output for this example
badones<-sapply(temp_orig,function(x) sum(x$density>10)) #which secnarios have lots of densities exceeding 10, triggering the density dpeendence problem so we can plot an example
which(badones>0)
#ok, 66 is really bad
this.pop=66 # example populatoin
temp_orig<-temp_orig[[this.pop]] #get 66th population

#get total pop
temp_orig$N<-temp_orig$Na+temp_orig$Nj

# grab a replicate illustrating density dependence failing
tempcase_orig<-temp_orig[temp_orig$i==42,]
#this is pop 66
#from pop df in main script, hectares is:

ha=864.8637*0.4047

#max density in scenario 5 is 4:

K<-ha*4 #this is the carrying capacity
#plot one example

#plot N
#Naplot<-ggplot(aes(x=j,y=Na),data=tempcase)+geom_line()+scale_y_continuous(trans="log10")+ geom_vline(xintercept = c(1:80)[tempcase$LAM>1.5]-1)+geom_hline(yintercept=K)

Nplot_orig<-ggplot(aes(x=j,y=N),data=tempcase_orig)+
  geom_line()+
  scale_y_continuous(trans="log10",labels=c("             100","1,000","10,000","100,000"),breaks=10^c(2:5))+
  #geom_vline(xintercept = c(1:80)[tempcase$LAM>1.5]-1)+
  geom_hline(yintercept=K)+
  ylab("Focal pop. size")+
  xlab("")

Nmplot_orig<-ggplot(aes(x=j,y=Nm),data=tempcase_orig)+
  geom_line()+
  scale_y_continuous(trans="log10",labels=c("10,000","100,000","1,000,000","10,000,000","100,000,000"),breaks=10^(4:8))+
  #geom_vline(xintercept = c(1:80)[tempcase$LAM>1.5]-1)
ylab("Metapop. size")+
  xlab("")

Nimmplot_orig<-ggplot(aes(x=j,y=Nimm+1),data=tempcase_orig)+
  geom_line()+
...
#fig_nodd<-annotate_figure(fig_nodd, top = text_grob("Density dependence broken",
...
#it's relatively rare for the density dependence to break, which means that the median value for the end focal pop size is generally not affected by the error and maybe why this was overlooked


# Figure 2 - example trajectory with DD fixed showing exponential growth -----------------------------


#look at it again but in the version with density dependence fixed (using as.numeric())
this.scen=5 # max density high
this.pop=66 # example population
temp<-readRDS(sprintf("dd/output/scen-%s.rds",this.scen))
temp<-temp[[this.pop]] #get 66th population
temp$N<-temp$Na+temp$Nj


#okay, it looks like it's always correct now.

#plot an example of the corrected version
tempcase<-temp[temp$i==36,]
#this is pop 66
#from pop df in main script, acres is: 864.8637
...
# Import data from original model run -------------------------------------
...
#subset just to the low management medium threats; this is the scenario most often interpreted in the discussion of folt et al.
df26<-df[df$scen==26,]


pops
# Figure S1 - metapops vary tremendously at end of scenario (scen 26) ---------------------------

#which "metapops" are in the same metapop?
# these are in order from 1:457; we don't use the objectid but just the order to keep track of them from the loop output
pops<-read.csv("pop_metapop_key.csv") #this is output from the pops object at the start of the model script
...
#implication is a maximum of 10-fold growth over 80 years- represents a generous upper limit
...
# Figure 3 - Which pops have unrealistic growth? --------
...
#Figure: change in pop size for each pop
...
######### get model outcomes and plot them #####
...
#Folt's way: take medians across replicates for each pop, then sum
orig_all %>%
  filter(j==80) %>% #just grab 80th year
  group_by(pop) %>% #group by population
  summarize(folt=median(Nj)+median(Na),sumfirst=median(Nj+Na)) %>% #calculate either sum of medians or median of sums of Njuv and Nadult for each pop
  select(folt,sumfirst) %>%
  colSums() #sum the medians for each pop
#these two approaches differ by around 900.  The one called folt is what is used in the original ms

#what i was thinking: sum pops for each rep then take medians across replicates
orig_all %>%
  filter(j==80) %>%
  mutate(N=Nj+Na) %>%
  group_by(i) %>% #group by replicate
  summarize(Ntot=sum(N)) %>% #sum pops for each replicate i
  pull(Ntot) %>% #pull the final column
  median

#the #s are quite different.
...
# Figure S3 - Total pop at end for each scenario --------------------------
...
p <- ggplot(data.frame(), aes(x = 1, y = 1)) +
  ...
  xlab("")+
  ylab("")
# Add table of scenario names to the plot
...
# Figure S4 - No. pops at end for all scenarios ---------------------------
...
#facet_wrap(~scen_num,ncol=8) +
 ...
 # ggtitle("Total population size at year 2100")+
  ...
  xlab("") #+
  #geom_errorbar(aes(ymin=NTottLCL, ymax=NTottUCL), width=.2,
...
#dummydata$lambda0.75<-dummydata$lambda0.75/sum(dummydata$lambda0.75)*1000
